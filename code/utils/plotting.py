...
def plot(exps, kind, log_scale=True, legend=None, file=None,
         x_label='epochs', y_label=None, last=True, min_value=0., title=None, bottom=None):
    fig, ax = plt.subplots()
    ...
    for i, exp in enumerate(exps):
        ...
        plot_mean_std(
            ax, runs, kind, i, label, min_value,
            partial_ratio=exp.partial_participation_ratio)
...
def plot_mean_std(ax, runs, metric_name, i, label, min_value=0.,
    ...
    x = np.array(runs[0][preffix + '_round']) * partial_ratio
