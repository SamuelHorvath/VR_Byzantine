# from: https://github.com/kuangliu/pytorch-cifar
"""ResNet in PyTorch.
For Pre-activation ResNet, see 'preact_resnet.py'.
Reference:
[1] Kaiming He, Xiangyu Zhang, Shaoqing Ren, Jian Sun
    Deep Residual Learning for Image Recognition. arXiv:1512.03385
"""
import torch
import torch.nn as nn
import torch.nn.functional as F
from torchvision import datasets, transforms


GROUP_NORM_LOOKUP = {
    16: 2,  # -> channels per group: 8
    32: 4,  # -> channels per group: 8
    64: 8,  # -> channels per group: 8
    128: 8,  # -> channels per group: 16
    256: 16,  # -> channels per group: 16
    512: 32,  # -> channels per group: 16
    1024: 32,  # -> channels per group: 32
    2048: 32,  # -> channels per group: 64
}


def create_norm_layer(num_channels, batch_norm=True):
    if batch_norm:
        return nn.BatchNorm2d(num_channels)
    return nn.GroupNorm(GROUP_NORM_LOOKUP[num_channels], num_channels)


class BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, batch_norm=True):
        super(BasicBlock, self).__init__()
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3,
            stride=stride, padding=1, bias=False)
        self.bn1 = create_norm_layer(planes, batch_norm)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn2 = create_norm_layer(planes, batch_norm)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False),
                create_norm_layer(self.expansion*planes, batch_norm)
            )

        self.relu = nn.ReLU()

    def forward(self, x):
        out = self.relu(self.bn1(self.conv1(x)))
        out = self.bn2(self.conv2(out))
        out += self.shortcut(x)
        out = self.relu(out)
        return out


class Bottleneck(nn.Module):
    expansion = 4

    def __init__(self, in_planes, planes, stride=1, batch_norm=True):
        super(Bottleneck, self).__init__()
        self.conv1 = nn.Conv2d(in_planes, planes, kernel_size=1, bias=False)
        self.bn1 = create_norm_layer(planes, batch_norm)
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=3,
                               stride=stride, padding=1, bias=False)
        self.bn2 = create_norm_layer(planes, batch_norm)
        self.conv3 = nn.Conv2d(planes, self.expansion *
                               planes, kernel_size=1, bias=False)
        self.bn3 = create_norm_layer(self.expansion*planes, batch_norm)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False),
                create_norm_layer(self.expansion*planes, batch_norm)
            )
        self.relu = nn.ReLU()

    def forward(self, x):
        out = self.relu(self.bn1(self.conv1(x)))
        out = self.relu(self.bn2(self.conv2(out)))
        out = self.bn3(self.conv3(out))
        out += self.shortcut(x)
        out = self.relu(out)
        return out


class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10, batch_norm=True):
        super(ResNet, self).__init__()
        self.in_planes = 64

        self.conv1 = nn.Conv2d(3, 64, kernel_size=3,
                               stride=1, padding=1, bias=False)
        self.bn1 = create_norm_layer(64, batch_norm)
        self.layer1 = self._make_layer(
            block, 64, num_blocks[0], stride=1, batch_norm=batch_norm)
        self.layer2 = self._make_layer(
            block, 128, num_blocks[1], stride=2, batch_norm=batch_norm)
        self.layer3 = self._make_layer(
            block, 256, num_blocks[2], stride=2, batch_norm=batch_norm)
        self.layer4 = self._make_layer(
            block, 512, num_blocks[3], stride=2, batch_norm=batch_norm)
        self.linear = nn.Linear(512*block.expansion, num_classes)
        self.relu = nn.ReLU()

    def _make_layer(self, block, planes, num_blocks, stride, batch_norm):
        strides = [stride] + [1]*(num_blocks-1)
        layers = []
        for stride in strides:
            layers.append(block(self.in_planes, planes, stride, batch_norm))
            self.in_planes = planes * block.expansion
        return nn.Sequential(*layers)

    def forward(self, x):
        out = self.relu(self.bn1(self.conv1(x)))
        out = self.layer1(out)
        out = self.layer2(out)
        out = self.layer3(out)
        out = self.layer4(out)
        out = F.avg_pool2d(out, 4)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
        return out


# TODO: Any way we can actually have an useful pretrained argument here?
def ResNet18(pretrained=False, num_classes=10):
    return ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes)


def ResNet34(pretrained=False, num_classes=10):
    return ResNet(BasicBlock, [3, 4, 6, 3], num_classes=num_classes)


def ResNet50(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 4, 6, 3], num_classes=num_classes)


def ResNet101(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 4, 23, 3], num_classes=num_classes)


def ResNet152(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 8, 36, 3], num_classes=num_classes)


def ResNet18gn(pretrained=False, num_classes=10):
    return ResNet(BasicBlock, [2, 2, 2, 2],
                  num_classes=num_classes, batch_norm=False)


def ResNet34gn(pretrained=False, num_classes=10):
    return ResNet(BasicBlock, [3, 4, 6, 3],
                  num_classes=num_classes, batch_norm=False)


def ResNet50gn(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 4, 6, 3],
                  num_classes=num_classes, batch_norm=False)


def ResNet101gn(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 4, 23, 3],
                  num_classes=num_classes, batch_norm=False)


def ResNet152gn(pretrained=False, num_classes=10):
    return ResNet(Bottleneck, [3, 8, 36, 3],
                  num_classes=num_classes, batch_norm=False)


class Net(nn.Module):
    # init Resnet18 with group norm
    def __init__(self):
        super(Net, self).__init__()
        self.resnet = ResNet18gn(num_classes=10)

    def forward(self, x):
        return self.resnet(x)


def cifar(
        data_dir,
        train,
        download,
        batch_size,
        shuffle=None,
        sampler_callback=None,
        dataset_cls=datasets.CIFAR10,
        drop_last=False,
        **loader_kwargs
):
    # if sampler_callback is not None and shuffle is not None:
    #     raise ValueError

    transorm_train = transforms.Compose([
        transforms.RandomCrop(32, padding=4),
        transforms.RandomHorizontalFlip(),
        transforms.ToTensor(),
        transforms.Normalize(
            (0.4914, 0.4822, 0.4465),
            (0.2023, 0.1994, 0.2010),
        ),
    ])

    transform_test = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(
            (0.4914, 0.4822, 0.4465),
            (0.2023, 0.1994, 0.2010),
        ),
    ])

    dataset = dataset_cls(
        data_dir,
        train=train,
        download=download,
        transform=transorm_train if train else transform_test,
    )

    sampler = sampler_callback(dataset) if sampler_callback else None

    return torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=shuffle,
        sampler=sampler,
        drop_last=drop_last,
        **loader_kwargs,
    )
